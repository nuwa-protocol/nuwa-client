...
function mapLitellmResponseToModels(
  ...
  return response.data.map((item) => ({
    ...
    provider: item.model_info.litellm_provider,
    model_slug: item.model_info.id,
    mode: item.model_info.mode,
    max: {
      tokens: item.model_info.max_tokens ?? null,
      input_tokens: item.model_info.max_input_tokens ?? null,
      output_tokens: item.model_info.max_output_tokens ?? null,
    },
    cost: {
      input_cost_per_million_token: item.model_info.input_cost_per_token
        ? item.model_info.input_cost_per_token * 1000000
        : null,
      output_cost_per_million_token: item.model_info.output_cost_per_token
