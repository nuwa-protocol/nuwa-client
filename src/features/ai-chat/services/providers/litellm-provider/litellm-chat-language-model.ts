  ...
  private getArgs({
    ...
    const baseArgs = {
      ...
      reasoning_effort:
        ...
        providerMetadata?.['litellm-compatible']?.reasoningEffort,
  ...
  async doGenerate(
    ...
    return {
      ...
      sources: responseBody.citations
        ? convertCitationsToSources(responseBody.citations)
        : undefined, // 从顶层 citations 获取并转换
    ...
    if (this.settings.simulateStreaming) {
      ...
      const simulatedStream = new ReadableStream<LanguageModelV1StreamPart>({
        start(controller) {
          ...
          // 将 citations 转换成 source 并 enqueue
          if (result.sources) {
      ...
      stream: response.pipeThrough(
        ...
        >({
          ...
          transform(chunk, controller) {
            ...
            // 将 citations 转换成 source 并 enqueue (only once):
            if (
              !hasSentSources &&
              ...
              hasSentSources = true;
              const sources = convertCitationsToSources(value.citations);
...
const LitellmChatResponseSchema = z.object({
  ...
  citations: z.array(z.string()).nullish(), // 添加 citations 字段到顶层
  choices: z.array(
    z.object({
      message: z.object({
...
// limited version of the schema, focussed on what is needed for the implementation
// this approach limits breakages when the API changes and increases efficiency
const createLitellmChatChunkSchema = <ERROR_SCHEMA extends z.ZodType>(
